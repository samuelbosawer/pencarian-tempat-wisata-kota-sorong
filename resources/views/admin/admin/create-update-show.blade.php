                ...
                <div class="row">
                    <div class="card mb-4">
                        <h5 class="card-header fw-bolder text-capitalize"><i class="menu-icon tf-icons bx bx-user"></i>
                            {{ $judul ?? 'Tambah Data Admin' }} </h5>
                        <div class="card-body">

                            @if (Request::segment(4) == 'ubah' && Request::segment(2) == 'admin')
                                <form action="{{ route('dashboard.admin.update', $data->id) }}" enctype="multipart/form-data"
                                    method="POST">
                                    @csrf
                                    @method('PUT')
                                @elseif (Request::segment(3) == 'tambah' && Request::segment(2) == 'admin')
                                    <form action="{{ route('dashboard.admin.store') }}" enctype="multipart/form-data"
                                        method="POST">
                                        @csrf
                                    @else
                                        <form action="">
...
{{-- @if (!Auth::user()->hasAnyRole(['admin', 'usaha','pengunjung'])) --}}
                            ...
                            <div class="col-md-12 mb-3 mx-auto">
                                    ...
                                    @if (Request::segment(3) == 'detail')
                                        <a href="{{ route('dashboard.admin.ubah', $data->id) }}"
                                            class="btn btn-dark text-white"> <i class="menu-icon tf-icons bx bx-pencil"></i>
                                            UBAH DATA </a>
                                    @elseif ((Request::segment(3) == 'tambah' || Request::segment(4) == 'ubah') && Request::segment(2) == 'admin')
                                        <button type="submit" class="btn btn-primary text-white">SIMPAN <i
                                                class="menu-icon tf-icons bx bx-save"></i></button>
                                    @endif

                                <a href="{{ route('dashboard.admin') }}" class="btn btn-dark text-white"> KEMBALI </a>
